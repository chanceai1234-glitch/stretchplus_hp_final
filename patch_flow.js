...
const replacement = `    <!-- Section 7: 施術の流れ -->
...
function replaceFlow(filename) {
    let html = fs.readFileSync(filename, 'utf8');
    const startPattern = '    <!-- Section 7: 施術の流れ -->';
    const endPattern = '    <!-- Section 8: FAQ -->';
    ...
    if (startIdx !== -1 && endIdx !== -1) {
        ...
        console.log("Updated flow in " + filename);
    } else {
        console.log("Could not find flow section in " + filename);
    }
}

replaceFlow('index.html');
replaceFlow('redesign/index.html');
